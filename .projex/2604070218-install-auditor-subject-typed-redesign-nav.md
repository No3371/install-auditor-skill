...
### Active Work
- **Subject-typed redesign (this nav)** — **Phases 0–1 complete; Phase 2 M2.1 + M2.2 + M2.3 + M2.4 complete (2026-04-12).** Four data primitives now live in `scripts/`: registry-lookup, typosquat-check, vuln-lookup, and dep-scan. **Remaining Phase 2 milestone: M2.5 (eval gate).** M2.4 artifacts are closed: proposal [`2604021203-transitive-dependency-auditing-proposal.md`](closed/2604021203-transitive-dependency-auditing-proposal.md), plan [`2604110900-transitive-dep-scan-plan.md`](closed/2604110900-transitive-dep-scan-plan.md), walkthrough [`2604110900-transitive-dep-scan-walkthrough.md`](closed/2604110900-transitive-dep-scan-walkthrough.md). M2.5 closes out Phase 2 with a regression pass + one PyPI typosquat eval case (pip/PyPI positive already covered by id 6).

### Known Blockers
- *(none)* — All prior blockers resolved. M2.5 has no external dependencies beyond the existing eval harness and closed Phase 2 artifacts.
...
**Current focus:** **Phase 2 M2.5 — eval gate.** M2.4 complete (2026-04-11): `scripts/dep-scan.ps1` shipped, §4.3/criteria addendum updated, eval ids 7+8 added (mkdirp@0.5.1 transitive CVE case + chalk@5.3.0 zero-dep clean case).
