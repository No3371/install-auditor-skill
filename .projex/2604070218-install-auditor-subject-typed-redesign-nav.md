...
> **Created:** 2026-04-07 | **Last Revised:** 2026-04-12
...
The skill is a working dispatcher with a fully populated registry-package pipeline. `SKILL.md` classifies subjects (Step 0), loads per-type workflows (Step 1), and applies the shared verdict tree + audit-coverage report (Step N). `workflows/registry-package.md` handles Type 1; the remaining 9 types fall back to `workflows/generic.md`. Four data primitives now exist: `scripts/registry-lookup.ps1` (registry metadata), `scripts/typosquat-check.ps1` (algorithmic name-similarity detection for npm), `scripts/vuln-lookup.ps1` (structured OSV + GHSA CVE queries for all 8 supported registry ecosystems), and `scripts/dep-scan.ps1` (Tier 2 direct-dep and Tier 3 npm transitive vulnerability scanning via OSV batch). The criteria layer is split: `references/criteria.md` (shared core, §4.3 now cross-references vuln-lookup + dep-scan) + `references/criteria/registry-package.md` (per-subject addendum now includes ecosystem trust signals, tier thresholds, install-script risk patterns, typosquat scoring, multi-DB CVE correlation, and transitive dependency scanning guidance). Eval coverage expanded to 10 cases (ids 0–9): M2.3 added lodash@4.17.20 CVE (id 5) + pip requests (id 6); M2.4 added mkdirp@0.5.1 transitive CVE detection (id 7) + chalk@5.3.0 clean dep-scan path (id 8); M2.5 added the PyPI name-confusion case `requers` vs `requests` (id 9) and dry-ran the full Phase 2 registry-package gate.

**Phase 2 is complete** — M2.1 (registry-package workflow), M2.2 (typosquat detection), M2.3 (multi-DB CVE correlation), M2.4 (transitive dependency auditing), and M2.5 (eval gate) are all complete as of 2026-04-12.
...
### Active Work
- **Subject-typed redesign (this nav)** — **Phases 0–2 complete (2026-04-12).** Registry-package is now the validated live path for known registry ecosystems, backed by four supporting data primitives in `scripts/`: registry-lookup, typosquat-check, vuln-lookup, and dep-scan. Phase 2 closed with eval coverage expanded to ids 0–9, including the PyPI `requers` vs `requests` name-confusion case and a manual dry-run regression pass across the Phase 2 registry-package set. **Current next action: Phase 3 M3.1 — author `workflows/browser-extension.md` and re-route the Wappalyzer control case.**
...
**Current focus:** **Phase 3 M3.1 — `workflows/browser-extension.md`.** Phase 2 closed on 2026-04-12 after the registry-package eval gate passed and eval coverage reached ids 0–9.

**Next up:** **Phase 3 M3.1** — author `workflows/browser-extension.md`, define subject-native browser-extension evidence/rubric rules, and re-route the Wappalyzer control case out of `generic.md`.
